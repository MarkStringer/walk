...
# -------------------------------------------------------
# Target line setup (takes current weight + date as arguments)
# Usage example:
#   gnuplot -e "last_weight=92.5; last_date='20250930'" newFormatWlkWt_weight_target_args.gnu
# -------------------------------------------------------

# If no argument given, set defaults
if (!exists("last_weight")) last_weight = 90
if (!exists("last_date"))   last_date = "20260101"

# Parse last_date argument as YYYYMMDD string
ld = strptime("%Y%m%d", last_date)

target_date   = strptime("%Y%m%d","20261231")
target_weight = 85

target(x) = last_weight + (target_weight-last_weight)*(x-ld)/(target_date-ld)
