...
set format x "%d-%b-%y"
set xdata time
set xrange ["20260101":"20261231"]
set yrange [85:120]
set y2range [-200:1800]
set y2tics "-200", 100, "1800"
set xtics font ", 16"
set xtics rotate by 45 right
set xtics nomirror "20260101",2592000, "20261231" 
set mxtics 4
set ytics nomirror 
...
p = 1
q = 1e-8
fit [strptime("%Y%m%d","20180101"):strptime("%Y%m%d","20261231")] g(x) recent u 1:2 via p,q	

h(x)= r+s*x
r = 1
s = 1e-8 
fit [strptime("%Y%m%d","20260101"):strptime("%Y%m%d","20261231")] h(x) "walk.csv" u 1:3 via r,s

i(x)= j+k*x
j = 1
k = 1e-8
fit [strptime("%Y%m%d","20180101"):strptime("%Y%m%d","20261231")] i(x) "recentWalk.csv" u 1:3 via j,k

j(x)=1460*(x - strptime("%Y%m%d","20260101"))/(strptime("%Y%m%d","20261231") - strptime("%Y%m%d","20260101"))
